...
void MeshField::Init()
{
	// 頂点バッファ生成
	{
		for (int x = 0; x <= TWENTY; x++)
		{
			for (int z = 0; z <= TWENTY; z++)
			{
				m_Vertex[x][z].Position = D3DXVECTOR3((x - 10) * 5.0f, g_FieldHieght[z][x], (z - 10) * -5.0f);
		...
		// 法線ベクトル算出
		for (int x = 1; x <= NINETEEN; x++)
		{
			for (int z = 1; z <= NINETEEN; z++)
			{
	...
	// インデックスバッファ生成
	{
		unsigned int index[(TWENTY_TWO * 2) * TWENTY - 2];
		...
		ZeroMemory(&bd, sizeof(bd));
		bd.Usage = D3D11_USAGE_DEFAULT;
		bd.ByteWidth = sizeof(unsigned) * ((TWENTY_TWO * 2) * TWENTY - 2);
